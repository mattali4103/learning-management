...
const ChinhSuaKHHTMau = () => {
  ...
  // Lấy pathname để xác định chế độ add
  const isAddMode = window.location.pathname.includes('/add/');

  // Lấy mã ngành và khóa học từ params, không cho phép đổi
  const maNganh = params.maNganh || "";
  const khoaHoc = params.khoaHoc || "";
  ...
  // Fetch ngành (chỉ để hiển thị tên ngành)
  ...
  const fetchChuongTrinhDaoTao = useCallback(async () => {
    ...
    try {
      const response = await axiosPrivate.get(
        HOCPHAN_SERVICE.CTDT_NGANH.replace(":khoaHoc", khoaHoc).replace(
          ":maNganh",
          maNganh
        )
      );
  ...
  const handleSave = async () => {
    if (!maNganh || !khoaHoc || selectedHocPhans.length === 0) {
  ...
  }, [maNganh, khoaHoc, fetchChuongTrinhDaoTao]);

  // Khi ở chế độ chỉnh sửa, tự động load các học phần đã có vào form
  useEffect(() => {
      ...
      if (maNganh && khoaHoc) {
        try {
          ...
          if (response.data && Array.isArray(response.data.data)) {
            setSelectedHocPhans(response.data.data.map((item : any, index: number) => ({
              ...item,
              id: `${item.hocPhan.maHp}-${item.hocKy?.maHocKy || 'unassigned'}-${index}`,
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-4 space-y-6">
        ...
        <PageHeader
          title={
            isAddMode
              ? "Thêm Kế hoạch Học tập Mẫu"
              : "Chỉnh sửa Kế hoạch Học tập Mẫu"
          }
          description={
            isAddMode
              ? "Thêm mới kế hoạch học tập mẫu cho chương trình đào tạo"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-100 text-gray-700">
                {danhSachNganh.find((n) => n.maNganh.toString() === maNganh)?.tenNganh || maNganh}
              </div>
            </div>
            {/* Hiển thị khóa học */}
            <div>
...
interface ChinhSuaHocPhanProps {
  pendingHocPhans: KeHoachHocTapDetail[];
  selectedHocPhans: KeHoachHocTapDetail[];
  pendingColumns: ColumnDef<KeHoachHocTapDetail>[];
  selectedColumns: ColumnDef<KeHoachHocTapDetail>[];
  statistics: {
    totalCredits: number;
    totalSubjects: number;
    totalSemesters: number;
    semesterGroups: Record<string, KeHoachHocTapDetail[]>;
  };
  handleConfirmAdd: () => void;
}

const ChinhSuaHocPhan = ({
  pendingHocPhans,
  selectedHocPhans,
  pendingColumns,
  selectedColumns,
...
interface ThemHocPhanProps {
  availableHocPhans: HocPhan[];
  pendingHocPhans: KeHoachHocTapDetail[];
  selectedHocPhans: KeHoachHocTapDetail[];
...
const ThemHocPhan = ({
  ...
  const filteredHocPhans = useMemo(() => {
    const allSelectedAndPendingIds = new Set([
    ...
    let filtered = availableHocPhans.filter(
      (hp) => !allSelectedAndPendingIds.has(hp.maHp)
    );
    ...
    return filtered;
  }, [availableHocPhans, searchTerm, selectedHocPhans, pendingHocPhans]);
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const isAlreadySelected =
          selectedHocPhans.some((item) => item.hocPhan.maHp === hocPhan.maHp) ||
          pendingHocPhans.some((item) => item.hocPhan.maHp === hocPhan.maHp);
        return (
          <div className="flex items-center justify-center">
            <button
              ...
              title={
                isAlreadySelected
                  ? "Đã có trong danh sách chờ hoặc kế hoạch"
